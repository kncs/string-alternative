...
var StringAlternative = module.exports = function() {
  ...
  self.above = function(trigger, alternative) {
    aboveAlternatives.push({
      trigger : trigger,
      alternative : alternative
    })
    return self;
  ...
  self._assignAlternativesAbove = function() {
    var status = false;
    if(aboveAlternatives.length > 0) {
      var tmp = aboveAlternatives
        .sort(function(a, b) {
          return b.trigger - a.trigger;
        })
        .reduce(function(current, above) {
          if(!status && (above.trigger < count)) {
            status = true;
            return above.alternative;
          }
          return current;
        }, '');
      if(status) str += tmp;
